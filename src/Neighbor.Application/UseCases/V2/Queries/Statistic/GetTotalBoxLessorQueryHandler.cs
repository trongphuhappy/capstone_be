...
public sealed class GetTotalBoxLessorQueryHandler : IQueryHandler<Query.GetTotalBoxLessor, Success<List<BoxLessorDTO>>>
{
    ...
    public async Task<Result<Success<List<BoxLessorDTO>>>> Handle(Query.GetTotalBoxLessor request, CancellationToken cancellationToken)
    {
        ...
        var result = await _dpUnitOfWork.StatisticRepositories.GetTotalBoxLessor(lessor.Id);
